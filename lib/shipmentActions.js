...
/**
 * Small helper: every write in this file used to just `await` the Supabase
 * call and move on, so an RLS rejection or any other DB error was silently
 * swallowed — the button's spinner would stop and nothing would happen, with
 * no error shown anywhere. This wraps a `{ data, error }` result and throws
 * a real Error when something went wrong, so every caller's existing
 * try/catch (or `.catch()`) can surface it to the user instead of failing
 * silently.
 */
...
/** Driver uploads (or re-uploads, after a resubmit request) the weighment slip ("kande ki parchi") during Documentation. */
export async function uploadWeighmentSlip({ load, file, driverId, merchantId }) {
  ...
  orThrow(
    await supabase
      .from("loads")
      .update({ weighment_slip_url: url, weighment_slip_status: "pending", weighment_slip_note: null })
      .eq("id", load.id)
  );
  triggerPush({
    userId: merchantId,
    title: "Weighment slip received",
    body: `${load.commodity} — driver uploaded the slip. Please review it.`,
...
/** Merchant approves the weighment slip — unlocks the Bilty form. */
export async function approveWeighmentSlip({ load, driverId }) {
  orThrow(await supabase.from("loads").update({ weighment_slip_status: "approved" }).eq("id", load.id));
  triggerPush({
    userId: driverId,
    title: "Weighment slip approved",
    body: `${load.commodity} — the merchant is now preparing your Bilty.`,
    url: "/driver/dashboard",
    tag: `load-${load.id}`,
  });
}

/** Merchant rejects the weighment slip and asks the driver to re-upload it, with an optional reason. */
export async function requestResubmitSlip({ load, driverId, note }) {
  orThrow(
    await supabase
      .from("loads")
      .update({ weighment_slip_status: "resubmit_requested", weighment_slip_note: note || null })
      .eq("id", load.id)
  );
  triggerPush({
    userId: driverId,
    title: "Please re-upload the weighment slip",
    body: note ? `${load.commodity} — ${note}` : `${load.commodity} — the merchant asked for a clearer weighment slip photo.`,
    url: "/driver/dashboard",
    tag: `load-${load.id}`,
    urgent: true,
  });
...
export async function markOnTheWay({ load, merchantId }) {
  orThrow(await setTripStage(load.id, 3));
  triggerPush({
    userId: merchantId,
...
export async function approveArrival({ load, driverId }) {
  orThrow(await supabase.from("loads").update({ merchant_approved_at: new Date().toISOString() }).eq("id", load.id));
  triggerPush({
    userId: driverId,
